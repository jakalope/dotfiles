...
class BeautifyBash:
    ...
    def beautify_string(self, data, path=''):
        ...
        for record in re.split('\n', data):
            ...
            test_record = re.sub(r'(<<-?\s*)["\']((?:\\.|[^"\\])*?)["\']',
                                 r'\1\2', stripped_record)
            ...
            if (in_here_doc):  # pass on with no changes
                ...
                if (in_ext_quote):
                    ...
                    else_case = (0, -1)[re.search(r'^(else\b|elif\b.*\bthen$)',
                                                  test_record) != None]
                    # an ad-hoc solution for the standalone ";;"
                    double_comma_case = (
                        0, +1)[re.search('^;;$', test_record) != None]
                    net = inc - outc
                    tab += min(net, 0)
                    ...
                    if (continued):
                        ...
                        if stripped_record:
                            ...
                            if stripped_record.startswith(
                                    '#') and last_if_indent == re.match(
                                        r'^\s*', record).group():
                                extab = max(0, extab - 1)
...
def main():
    ...
    parser.add_argument(
        ...
        metavar='TAB',
        type=int,
        help='Tab size (default: 4)',
        default=4)
    parser.add_argument(
        '--no-backup',
        '-n',
        action='store_true',
        help='Do not backup file before overwrite',
        default=False)
    parser.add_argument(
        'files',
        metavar='FILE',
