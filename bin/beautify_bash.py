        ...
        for record in re.split("\n", data):
            ...
            if in_here_doc:  # pass on with no changes
                ...
                if in_ext_quote:
                    ...
                    else_case = (0, -1)[
                        re.search(r"^(else\b|elif\b.*\bthen$)", test_record) != None
                    ]
                    ...
                    if continued:
                        ...
                        if stripped_record:
                            ...
                            if (
                                stripped_record.startswith("#")
                                and last_if_indent == re.match(r"^\s*", record).group()
                            ):
                                extab = max(0, extab - 1)
                            output.append(
...
def main():
    ...
    parser.add_argument(
        "--tab-str", "-c", metavar="CHAR", help='Tab string (default: " ")', default=" "
    )
    parser.add_argument(
        "--tab-size",
        "-t",
    ...
    parser.add_argument(
        "files", metavar="FILE", nargs="+", help='Filename (use "-" for stdin)'
    )
