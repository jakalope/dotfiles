...
def main(argv):
    ...
    extroot = os.path.join(os.path.dirname(thisdir), 'hgwatchman')
    extopts = ['--extra-config-opt', 'extensions.hgwatchman=%s' % extroot,
               '--extra-config-opt', 'watchman.mode=on']
    if not options.disable_blacklist:
        extopts += ['--blacklist', os.path.join(thisdir, 'blacklist')]

    cwd = os.path.join(options.hg, 'tests')
